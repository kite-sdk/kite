...
public class TestTarImportCommand {
    ...
    private static final String TAR_TEST_GZIP_FILE = TEST_DATA_DIR + "/test.tar.gz";
    private static final String TAR_TEST_GZIP_ROOT_PREFIX = "testgzip";
    private static final String TAR_TEST_BZIP2_FILE = TEST_DATA_DIR + "/test.tar.bz2";
    ...
    public static void createTestInputFiles() throws IOException {
        ...
        try {
            // No compression
            tosNoCompression = new TarOutputStream(testFS.create(new Path(TAR_TEST_FILE), true));
            rootDirHeader = TarHeader.createHeader(TAR_TEST_ROOT_PREFIX, 0, System.currentTimeMillis(), true);
            tosNoCompression.putNextEntry(new TarEntry(rootDirHeader));

            // Gzip compression
            tosGzipCompression = new TarOutputStream(new GzipCompressorOutputStream(testFS.create(new Path(TAR_TEST_GZIP_FILE), true)));
            rootDirHeader = TarHeader.createHeader(TAR_TEST_GZIP_ROOT_PREFIX, 0, System.currentTimeMillis(), true);
            tosGzipCompression.putNextEntry(new TarEntry(rootDirHeader));

            // BZip2 compression
            tosBzip2Compression = new TarOutputStream(new BZip2CompressorOutputStream(testFS.create(new Path(TAR_TEST_BZIP2_FILE), true)));
            rootDirHeader = TarHeader.createHeader(TAR_TEST_BZIP2_ROOT_PREFIX, 0, System.currentTimeMillis(), true);
            ...
            for (int i = 0; i < NUM_TEST_FILES; ++i) {
                ...
                TestTarImportCommand.writeToTarFile(tosNoCompression, TAR_TEST_ROOT_PREFIX + "/" + fName, fContent);
                TestTarImportCommand.writeToTarFile(tosGzipCompression, TAR_TEST_GZIP_ROOT_PREFIX + "/" + fName, fContent);
                TestTarImportCommand.writeToTarFile(tosBzip2Compression, TAR_TEST_BZIP2_ROOT_PREFIX + "/" + fName, fContent);
    ...
    private static void writeToTarFile(TarOutputStream tos, String name, String content) throws IOException {
        TarHeader tarHeader = TarHeader.createHeader(name, content.length(), System.currentTimeMillis(), false);
    ...
    public void testTarBzip2ImportCommand() throws IOException {
        command.targets = Lists.newArrayList(TAR_TEST_BZIP2_FILE, TEST_DATASET_NAME);
    ...
    public void setup() throws Exception {
      TestUtil.run("-v", "create", TEST_DATASET_NAME,
              ...
              "-s", "kite-tools-parent/kite-tools/src/test/resources/test-schemas/tar-import.avsc");
