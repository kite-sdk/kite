...
public class Constraints implements Serializable{
  ...
  private Range readRangePredicate(ObjectInputStream in) throws IOException{
    ...
    if (in.readBoolean()) {
      ...
      Comparable lowerBound = (Comparable) readValue(in);
    ...
    if (in.readBoolean()) {
      ...
      Comparable upperBound = (Comparable) readValue(in);
  ...
  private Object readValue(ObjectInputStream in) throws IOException {
    ...
    if (isAvro) {
      ...
      Schema schema = new Parser().parse(in.readUTF());
      ...
      Object avroTarget = createAvroTarget(className, schema);
   ...
   * @param schema the schema of the target object
   * @return an instance of Avro class used for deserializing.
   * @throws IOException error creating a target.
   */
  private Object createAvroTarget(String className, Schema schema) throws IOException {
      try{
        if(GenericData.Record.class.getName().equals(className)){
