...
import org.kitesdk.data.morphline.MorphlineDatasetReader;
...
public class TestMorphlineCSVFileReader extends TestDatasetReaders<GenericData.Record> {
  
  private static final File RESOURCES_DIR = new File("target/test-classes/test-morphlines");
  ...
  public DatasetReader<GenericData.Record> newReader() throws IOException {
    String morphlineFileContents = Files.toString(new File(RESOURCES_DIR, "readCSV.conf"), Charsets.UTF_8);
    final DatasetDescriptor desc = new DatasetDescriptor.Builder()
        .property(MorphlineDatasetReader.MORPHLINE_FILE_CONTENTS_PARAM, morphlineFileContents)
        .schema(VALIDATOR_SCHEMA)
        .build();
    return new MorphlineDatasetReader<GenericData.Record>(localfs, validatorFile, desc);
  }
  ...
  public void testRejectsNonRecordSchemas() {
    final DatasetDescriptor desc = new DatasetDescriptor.Builder()
        .property(MorphlineDatasetReader.MORPHLINE_FILE_PARAM, RESOURCES_DIR + "/readCSV.conf")
        .schema(SchemaBuilder.array().items().stringType())
        .build();
    new MorphlineDatasetReader(localfs, csvFile, desc);
  }

  @Test
  public void testStringSchema() {
    final DatasetDescriptor desc = new DatasetDescriptor.Builder()
        .property(MorphlineDatasetReader.MORPHLINE_FILE_PARAM, RESOURCES_DIR + "/readCSVWithStringSchema.conf")
        .schema(STRINGS)
        .format(Formats.fromString("morphline"))
        .build();
    
    final DatasetReader<GenericData.Record> reader =
        new MultiFileDatasetReader<GenericData.Record>(localfs, Arrays.asList(csvFile), desc);
        //new MorphlineDatasetReader<GenericData.Record>(localfs, csvFile, desc);
  ...
  public void testTSV() {
    final DatasetDescriptor desc = new DatasetDescriptor.Builder()
        .property(MorphlineDatasetReader.MORPHLINE_FILE_PARAM, RESOURCES_DIR + "/readTSV.conf")
        .schema(STRINGS)
        .build();
    final DatasetReader<GenericData.Record> reader =
        new MorphlineDatasetReader<GenericData.Record>(localfs, tsvFile, desc);
    ...
    Assert.assertFalse(reader.hasNext());
    Assert.assertTrue(reader.toString().length() > 0);
  }

  @Test
  public void testNormalSchema() {
    final DatasetDescriptor desc = new DatasetDescriptor.Builder()
        .property(MorphlineDatasetReader.MORPHLINE_FILE_PARAM, RESOURCES_DIR + "/readCSVWithNormalSchema.conf")
        .schema(SCHEMA)
        .build();
    final DatasetReader<GenericData.Record> reader =
        new MorphlineDatasetReader<GenericData.Record>(localfs, csvFile, desc);
    ...
    Assert.assertFalse(reader.hasNext()); // because of missing default
    Assert.assertFalse(reader.hasNext());
  }
  ...
  public void testReflectedRecords() {
    final DatasetDescriptor desc = new DatasetDescriptor.Builder()
        ...
        .schema(BEAN_SCHEMA)
        .build();
    final MorphlineDatasetReader<TestBean> reader =
        new MorphlineDatasetReader<TestBean>(localfs, csvFile, desc);
