...
public class TestSchemaCommand {
  ...
  public void testSchemaStdout() throws Exception {
    ...
    verify(console).info(schema.toString(true /* pretty print by default */));
  ...
  public void testMultipleSchemasStdout() throws Exception {
    ...
    verify(console).info(anyString(),
        eq("users"), eq(schema.toString(true /* pretty print by default */)));
    verify(console).info(anyString(),
        eq("users_2"), eq(schema.toString(true /* pretty print by default */)));
    verify(console).trace(contains("repo:hive"));
    verifyNoMoreInteractions(console);
  }
  ...
  public void testSchemaToFile() throws Exception {
    ...
    Assert.assertEquals("File should contain pretty printed schema",
        fileContent, schema.toString(true /* pretty print by default */));
    verify(console).trace(contains("repo:hive"));
    verifyNoMoreInteractions(console);
  }
  ...
  public void testMinimize() throws Exception {
    ...
    int rc = command.run();
    Assert.assertEquals("Should return success code", 0, rc);
    verify(console).info(schema.toString(false /* do not pretty print */));
    verify(console).trace(contains("repo:hive"));
    verifyNoMoreInteractions(console);
  }
